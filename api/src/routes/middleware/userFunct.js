...
const getUser = async (mail, fullName) => {
    try {
        let databaseUser;

        if (mail) {
            databaseUser = await modelgetUserFromDatabase(mail);
        } else if (fullName) {
            databaseUser = await modelgetAllUserFromDatabasebyName(fullName);
        } else {
            databaseUser = await modelgetAllUserFromDatabase();
        }
