...
const DashboardCard = (props) => {
  ...
  const [checked, setChecked] = useState(props.element.isActive);
  const dispatch = useDispatch();

  useEffect(() => {
    localStorage.setItem(`isActive-${props.element.id}`, checked.toString());
        ...
        <Box sx={{ height: "80%", display: "flex" }}>
          <Box sx={{ width: "50%" }}>
            ...
            <Accordion sx={{ margin: "1%" }}>
              ...
              <AccordionDetails>{props.element.description}</AccordionDetails>
            </Accordion>
          </Box>
